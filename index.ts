...
export const rsync = (
    ...
    const rsyncBinPath = options?.binPath || 'rsync';
    const rsyncArgs: string[] = [];

    if (options?.rsyncOptions) {
        // https://github.com/Microsoft/TypeScript/issues/12870
        for (const key of Object.keys(options.rsyncOptions) as (keyof RsyncOptions)[]) {
            const flagName = RsyncOptionsMap[key];
            const flagValue = options.rsyncOptions[key];

            if (typeof flagValue === 'boolean' && flagValue) {
                rsyncArgs.push(flagName);
            } else if (typeof flagValue === 'string' || typeof flagValue === 'number') {
                rsyncArgs.push(flagName, flagValue.toString());
            }
        }
    }
    const proc = execa(
        rsyncBinPath,
        [...rsyncArgs, ...source, destination, '--info=progress2'],
        { lines: true },
    );

    return new Promise<RsyncResult>(async (resolve, reject) => {
        ...
        for await (const line of proc.iterable({ from: 'stdout' })) {
            ...
            if (options?.onProgress) {
                const progress = parseOutputLine(line);
                if (progress) {
...
const parseOutputLine = (line: string): RsyncProgress | null => {
    ...
    const match = line.match(/(?<bytes>[\d,]+)\s+(?<percent>\d+)%\s+(?<speedString>(?<speedBytes>[\d\\.]+)\w{2,3}\/s)\s+(?<eta>[\d:]+)(?<isBadLine>\s+\(?xfr#(?<xfr>\d+),\s+(?<scanning>ir|to)-chk=(?<queued>\d+)\/(?<total>\d+))\)?/);
    if (match?.groups && !match.groups.isBadLine) {
        const {
            bytes,
            percent,
            speedString,
        ...
        const etaSeconds = seconds + minutes * 60 + hours * 60 * 60;

        return {
            percentage: Number(percent),
            transferred: Number(bytes.replaceAll(',', '')),
            eta: etaSeconds,
            speed: parse(speedString),
            xfr: Number(xfr),
            isScanning: scanning === 'ir',
            totalFiles: Number(total),
            queuedFiles: Number(queued),
            runtime: 0,
        };
    }
...
export const RsyncOptionsMap: { [key in keyof Required<RsyncOptions>]: string } = {
    noMotd: '--no-motd',
    checksum: '--checksum',
    archive: '--archive',
    noOption: '--no-OPTION',
